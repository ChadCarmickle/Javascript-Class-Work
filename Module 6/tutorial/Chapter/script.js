...
// declare global variables. for set up page. 
var zIndexCounter; 
var pos = []; 
var origin; 
   ...
   var movableItems = document.querySelectorAll("#room div");
	...
	for (var i = 0; i < movableItems.length; i++) { 
		...
		if (movableItems[i].addEventListener) { 
			movableItems[i].addEventListener("movedown", startDrag, false); 
...
function startDrag(evt){ 
	...
	this.addEventListener("movemove", moveDrag, false); 
...
if (evt.targetTouch = evt.targetTouches.length) { 
	var thisTouch = evt.targetTouch[0]; 
...
function removeTouchListener() { 
	this.removeEventListener("touchmose", moveDrag, false); 
